...
### 1. Entrada Inicial
Ao iniciar o bot ou um novo ciclo, ele coloca uma ordem de entrada baseada no **Tipo de Entrada** configurado:

- **Limite (padrão)**: Coloca uma ordem limite no seu `entry_price` exato. Estes preços são definidos pelo usuário e persistem independentemente para todos os símbolos configurados.
- **Mercado**: Executa a entrada imediatamente aos preços de mercado atuais.
- **Condicional (Cond. Limit / Cond. Market)**: Aguarda o preço atingir um "Preço de Gatilho" antes de colocar uma ordem Limite (ao Preço da Ordem) ou executar uma ordem a Mercado.

O tamanho total da posição é calculado como:
...
Após o preenchimento da entrada inicial, o bot coloca imediatamente **toda a grade de TP como um lote** de ordens Limite no livro da exchange. Isso garante que sua estratégia de saída esteja ativa no momento em que você tiver uma posição.
...
- **Abas de Tipo de Entrada:** Escolha entre estratégias de entrada **Limite**, **Mercado** ou **Condicional**.
- **Preço de Entrada / Gatilho:** Para ordens Limite e Condicionais, defina seus preços alvo específicos. Estes preços persistem por símbolo e são salvos automaticamente.
...
#### Aba Console
Log em tempo real de toda a atividade do bot. Ele fornece feedback aprimorado, incluindo:
- **Logs Localizados**: Totalmente traduzidos em Português ou Inglês.
- **Identificação de Conta**: Cada entrada de log é prefixada com o nome da conta (ex: `[Account 1]`).
- **Rastreamento Detalhado de Ordens**: Registra explicitamente tipos de ordens, quantidades e preços (ex: `Colocando ordem BUY LIMIT para BTCUSDC: Qtd 0.025 @ Preço 50001`).
- **Confirmação de Execução**: Confirmação em tempo real da execução de ordens e colocação subsequente da grade.
